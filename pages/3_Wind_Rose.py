...
df.rename(columns={'co2_flux': 'CO2 (\u03bcmol/m2/s)'}, inplace=True)

if day and not night:
    st.write("You selected daytime.")
    df = df[df.daytime == 1]
    df = df[df['CO2 (\u03bcmol/m2/s)'] <= 0]
    
    fig = px.scatter_polar(df, r="wind_speed", theta="wind_dir",
    color="CO2 (\u03bcmol/m2/s)", color_discrete_sequence=px.colors.sequential.Plasma_r)
	...
	fig = px.scatter_polar(df, r="wind_speed", theta="wind_dir", color="CO2 (\u03bcmol/m2/s)", color_discrete_sequence=px.colors.sequential.Plasma_r)
